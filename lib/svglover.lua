...
function svglover.load(svgfile, options)
    ...
    -- extract SVG root element attributes for inheritance (fill, stroke, etc.)
    local svg_tag = string.match(file_contents, "<svg[^>]*>")
    if svg_tag then
        local root_attrs = svglover._getattributes(svg_tag)
        for attr, _ in pairs(svglover._inherited_attributes) do
            if root_attrs[attr] then
                state.parent.attributes[attr] = root_attrs[attr]
            end
        end
    end
...
function svglover._colorparse(str, default_r, default_g, default_b, default_a)
    ...
    -- currentColor: return white so callers can tint via love.graphics.setColor
    if str == "currentColor" then
        return 1, 1, 1, 1
    end
...
function svglover._gensubpath(element, vertices, closed, extdata, options)
    ...
    if f_red ~= nil and vertexcount >= 6 then
        if options.use_love_fill == true then
            ...
            -- LÖVE 12: use use_love_fill path since stencil API changed
            result = result ..
                "love.graphics.setColor(" .. f_red .. "," .. f_green .. "," .. f_blue .. "," .. (f_alpha * f_opacity * opacity) .. ")\n" ..
                "love.graphics.polygon(\"fill\", extdata[" .. bufferid .. "])\n"
        end
...
svglover._elementsfunctions["rect"] = function(state, element, extdata, options)
    ...
    local rx = svglover._getattributevalue(element, "rx", "0")
    local ry = svglover._getattributevalue(element, "ry", rx)
    ...
    local has_round = tonumber(rx) > 0 or tonumber(ry) > 0
    local rect_suffix = has_round and ("," .. rx .. "," .. ry .. ")\n") or (")\n")

    if f_red ~= nil then
        result = result .. "love.graphics.setColor(" .. f_red .. "," .. f_green .. "," .. f_blue .. "," .. (f_alpha * f_opacity * opacity) .. ")\n"
        result = result .. "love.graphics.rectangle(\"fill\"," .. x_offset .. "," .. y_offset .. "," .. width .. "," .. height .. rect_suffix
    end

    if s_red ~= nil then
        result = result .. "love.graphics.setColor(" .. s_red .. "," .. s_green .. "," .. s_blue .. "," .. (s_alpha * s_opacity * opacity) .. ")\n"
        result = result .. "love.graphics.rectangle(\"line\"," .. x_offset .. "," .. y_offset .. "," .. width .. "," .. height .. rect_suffix
    end
...
svglover._elementsfunctions["line"] = function(state, element, extdata, options)
    local x1 = svglover._getattributevalue(element, "x1", "0")
    local y1 = svglover._getattributevalue(element, "y1", "0")
    local x2 = svglover._getattributevalue(element, "x2", "0")
    local y2 = svglover._getattributevalue(element, "y2", "0")

    local s_red, s_green, s_blue, s_alpha = svglover._colorparse(svglover._getattributevalue(element, "stroke"))
    local opacity = tonumber(svglover._getattributevalue(element, "opacity", "1"), 10)
    local s_opacity = tonumber(svglover._getattributevalue(element, "stroke-opacity", "1"), 10)
    local linewidth = svglover._getattributevalue(element, "stroke-width", "1")

    local result = ""
    if s_red ~= nil then
        result = result .. "love.graphics.setColor(" .. s_red .. "," .. s_green .. "," .. s_blue .. "," .. (s_alpha * s_opacity * opacity) .. ")\n"
        result = result .. "love.graphics.setLineWidth(" .. linewidth .. ")\n"
        result = result .. "love.graphics.line(" .. x1 .. "," .. y1 .. "," .. x2 .. "," .. y2 .. ")\n"
    end
